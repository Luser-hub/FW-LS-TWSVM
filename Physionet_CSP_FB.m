...
for subj = 1:20
    ...
    fixed_bandwidth_file_name = ['D:\Download\pyproject\testEEGNet\Data\physionet\fixed_bandwidth\subj_', num2str(subj),'_decomp.mat'];
    ...
    if ~exist(fixed_bandwidth_file_name,'file')
        ...
        for trial = 1:data_num_dimensions(1)
            ...
            for channel =1:data_num_dimensions(2)
                for i =1:4
                    trial_data(channel,:,i)=bandpass_filter_within_range(data(trial,channel,:),low_freq(i),high_freq(i),160);
                end
            end
...
% file_name = ['Physionet_CSP_FB.xlsx'];
