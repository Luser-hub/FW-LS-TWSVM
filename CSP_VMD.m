...
for subj = 1:25
    % 存储分解后的VMD文件
    VMD_file_name = ['D:\Download\pyproject\testEEGNet\Data\19228725\VMD_files\subj_', num2str(subj),'_VMDdecomp.mat'];
    % 确定保留的特征个数
    numberOfFeatures = 16;
    if ~exist(VMD_file_name,'file')
        ...
        for ses = 1:5
            ...
            % 把当前被试所有session的数据拼接到一起
            if isempty(subj_data_array)
                subj_data_array = data_subset; 
            else
                subj_data_array = cat(1, subj_data_array, data_subset);
            end
            
            % 把当前被试所有session的标签拼接到一起
            if isempty(subj_label_array)
                subj_label_array = labels; 
            else
                subj_label_array = cat(2, subj_label_array, labels);
            end
        ...
        % 对于所有数据，进行VMD分解操作
        all_decomp_data = zeros(data_num_dimensions(1),32, 500, 8);
...
%         random_indices = randperm(sample_dim);        
        ...
        for i = 1:8
            EEGSignal = struct('x',reshape(result{i},[500,32,all_data_shape(1)]),'y',subj_label_array);
