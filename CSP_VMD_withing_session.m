...
%         
%         % 定义文件名和保存路径
%         saved_filename = ['SH_32_subj',num2str(subj),'_ses',num2str(ses),'feature.mat'];  % 文件名
%         saved_filepath = 'D:\Download\pyproject\testEEGNet\Data\19228725\all32fetures\SH\';  % 文件保存路径
%         
%         % 将数据保存到 mat 文件中
...
% 
%         acc_result(subj,ses) = maxValue;
%         f1_result(subj,ses) = f1_turn(maxIndex,1);
...
%         disp(['Subject: ',num2str(subj),', Session: ',num2str(ses)]);
%         disp(['5-Fold CV F1 Score: ', num2str(F1_score)]);
%         disp(['5-Fold CVAccuracy: ', num2str(accuracy)]);
...
% file_name = ['withing_session_816_acc_result_imf',num2str(imf_num),'.xlsx'];
% xlswrite(file_name,acc_result);
% file_name = ['withing_session_816_f1_result_imf',num2str(imf_num),'.xlsx'];
